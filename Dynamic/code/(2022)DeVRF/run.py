...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, data_dict, stage, xyz_min=None, xyz_max=None, coarse_ckpt_path=None):
    ...
    for global_step in trange(start, cfg_train.N_iters):
        ...
        if cfg_train.ray_sampler in ['flatten', 'in_maskcache']:
            ...
            sel_b = torch.randint(rgb_tr.shape[0], [cfg_train.N_rand]).to('cpu')
            sel_r = torch.randint(rgb_tr.shape[1], [cfg_train.N_rand]).to('cpu')
            sel_c = torch.randint(rgb_tr.shape[2], [cfg_train.N_rand]).to('cpu')
